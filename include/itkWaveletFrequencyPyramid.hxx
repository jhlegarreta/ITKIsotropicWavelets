 ...
 *
 *=========================================================================*/
#ifndef itkWaveletFrequencyPyramid_hxx
#define itkWaveletFrequencyPyramid_hxx
...
WaveletFrequencyPyramid< TInputImage, TOutputImage>
...
::OutputIndexToLevelBand(unsigned int linear_index)
{
  ...
  typedef FixedArray<unsigned int, 2> ArrayBandLevelType;
  ArrayBandLevelType bounds;
  bounds[0] = this->m_HighPassSubBands;
  bounds[1] = this->m_Levels;
  ArrayBandLevelType band_level = itk::Ind2Sub(linear_index, bounds);

  // Low pass (band = 0).
  if (linear_index == this->GetTotalOutputs() - 1 )
  ...
  unsigned int band = band_level[0];
  unsigned int level = band_level[1];
  return std::make_pair(level, band);
};

template <typename TInputImage, typename TOutputImage>
std::vector< typename WaveletFrequencyPyramid<TInputImage, TOutputImage>::OutputImagePointer>
WaveletFrequencyPyramid<TInputImage, TOutputImage>
::GetOutputs()
{
...
::GetOutputsHighPass()
{
  // lowpass is the last
  std::vector<OutputImagePointer> outputPtrs;
  for( unsigned int nout = 0; nout < this->m_TotalOutputs - 1; ++nout)
    {
    outputPtrs.push_back(this->GetOutput(nout));
    }
  return outputPtrs;
...
template <typename TInputImage, typename TOutputImage>
typename WaveletFrequencyPyramid<TInputImage, TOutputImage>::OutputImagePointer
WaveletFrequencyPyramid<TInputImage, TOutputImage>
::GetOutputLowPass()
{
...
std::vector< typename WaveletFrequencyPyramid<TInputImage, TOutputImage>::OutputImagePointer>
WaveletFrequencyPyramid<TInputImage, TOutputImage>
::GetOutputsHighPassBandsByLevel(unsigned int level)
{
  std::vector<OutputImagePointer> outputPtrs;
  unsigned int nout_start = level * this->m_HighPassSubBands;
  unsigned int nout_end = (level + 1) * this->m_HighPassSubBands;
  if (nout_end > this->m_TotalOutputs - 1)
    nout_end = this->m_TotalOutputs - 1;
...
template <typename TInputImage, typename TOutputImage>
unsigned int WaveletFrequencyPyramid<TInputImage, TOutputImage>
...
template <typename TInputImage, typename TOutputImage>
void WaveletFrequencyPyramid<TInputImage, TOutputImage>
...
::GenerateOutputInformation()
{
  ...
  // TODO document that inputOrigin and inputSpacing is lost and should be restored at the end of the inverse wavelet transform.
  ...
  for (unsigned int level = 0; level < this->m_Levels; ++level)
    ...
    for (unsigned int band = 0; band < this->m_HighPassSubBands; ++band)
      {
      // low pass is the last output.
      unsigned int current_output = level * this->m_HighPassSubBands + band;
      // Set to the output
    ...
    if(level == this->m_Levels - 1 )
      {
      outputPtr = this->GetOutput(this->GetTotalOutputs() - 1);
      if (!outputPtr) continue;
      typename OutputImageType::RegionType largestPossibleRegion;
    ...
    for (unsigned int level = 0; level < this->m_Levels; ++level)
      ...
      for (unsigned int idim = 0; idim < TOutputImage::ImageDimension; idim++)
        ...
        if (outputSize[idim] < 1)
          itkExceptionMacro(<< "Failure at level: " << level << " in pyramid wavelet, going to negative image size. Too many levels for input image size.")
        }
      ...
      if(level == this->m_Levels - 1 )
        {
        unsigned int n_output = this->GetTotalOutputs() - 1;
        if (n_output == refIndex) continue;
        if (!this->GetOutput(n_output)) continue;
        outputRegion.Crop(this->GetOutput(n_output)->GetLargestPossibleRegion());
...
//       // This is to normalize the multi-band approach. TODO is this generic? or depend on wavelet? Related with sub-band dilations. 2^(1/k) instead of Dyadic dilations. In any case it should be decoupled from this and put it in the WaveletFilterBankGenerator.
...
//       multiplyByAnalysisBandFactor->SetConstant(std::pow(2.0, ( -(int)level - band/static_cast<double>(this->m_HighPassSubBands) )*ImageDimension/2.0 ) );
