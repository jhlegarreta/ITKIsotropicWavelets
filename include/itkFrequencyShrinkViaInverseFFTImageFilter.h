 ...
 * The problem with other frequency shrinker is that it applies a low-pass rectangle filter (chop-off frequencies). In the spatial domain this is equivalent to sinc interpolation (without any window), and it generates ringes (Gibbs effect)
 * A way to improve this would be a frequency interpolator, avoiding ringing, not much frequency leakeage, and a sharp cutoff to avoid losing much high frequency info.
 *
...
public:
  ...
  typedef itk::ShrinkImageFilter<typename InverseFFTFilterType::OutputImageType, typename InverseFFTFilterType::OutputImageType> ShrinkFilterType;
